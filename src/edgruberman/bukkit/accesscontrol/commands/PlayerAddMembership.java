 ...
 */
public class PlayerAddMembership implements CommandExecutor {

    private Plugin plugin;
    private AccountManager manager;

    public PlayerAddMembership(final JavaPlugin plugin, final String label, final AccountManager manager) {
    ...
    public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] args) {
        ...
        if (!user.addMembership(group)) {
            sender.sendMessage(ChatColor.WHITE + user.getName() + ChatColor.BLUE + " is already a direct member of " + ChatColor.WHITE + group.getName());
            return true;
        }
        ...
        if (user.getAttachment() != null) ((Player) user.getAttachment().getPermissible()).sendMessage(ChatColor.WHITE + senderName + ChatColor.GRAY + " added you to group: " + ChatColor.WHITE + this.manager.formatName(group));
        sender.sendMessage(ChatColor.WHITE + user.getName() + ChatColor.GREEN + " added as a direct member of " + ChatColor.WHITE + this.manager.formatName(group));
        return true;
    }
