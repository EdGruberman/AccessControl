...
public class Add implements CommandExecutor {
    ...
    public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] args) {
        ...
        if (group.isMember(principal, ( world != null ? world.getName() : null ))) {
            Main.courier.send(sender, "add-already", principal.getName(), group.getName(), ( world == null ? "server" : world.getName() ));
            return false;
        }

        group.addMember(principal, ( world != null ? world.getName() : null ));
        ((Main) this.plugin).save();
        Main.courier.send(sender, "add", principal.getName(), group.getName(), ( world == null ? "server" : world.getName() ));
        return true;
    }
